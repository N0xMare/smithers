...
describe("DB migration edges", () => {
  ...
  test("0035 repairs missing agent checkpoint tables and index idempotently", () => {
    const { sqlite, db } = setupMemoryDb();
    try {
      sqlite.run("DROP TABLE _smithers_agent_checkpoints");
      sqlite.run("DROP TABLE _smithers_agent_checkpoint_contents");
      sqlite.run("DELETE FROM _smithers_schema_migrations WHERE id = '0035_agent_checkpoints'");
      ensureSmithersTables(db);
      expect(
        sqlite
      ...
      expect(migrationRows(sqlite).filter((row) => row.id === "0035_agent_checkpoints")).toHaveLength(1);
    } finally {
      sqlite.close();
    }
  });

  test("0035 fails closed when a recorded checkpoint table has an incompatible shape", () => {
    const { sqlite, db } = setupMemoryDb();
    try {
      sqlite.run("DROP TRIGGER _smithers_agent_checkpoints_attempt_delete");
      ...
      expect(() => ensureSmithersTables(db)).toThrow(/0035 agent checkpoint schema mismatch/);
    } finally {
      sqlite.close();
    }
  });
