...
function assertAgentCheckpointSqliteSchema(sqlite) {
  for (const [table, shape] of Object.entries(AGENT_CHECKPOINT_TABLE_SHAPES)) {
    if (!tableExists(sqlite, table)) throw new Error(`0035 agent checkpoint schema is missing table ${table}`);
    ...
    if (
      ...
      throw new Error(
        `0035 agent checkpoint schema mismatch for ${table}: expected canonical columns and primary key ${shape.primaryKey.join(",")}`,
      );
    }
  }
  ...
  if (!agentCheckpointSqliteForeignKeysMatch(fks)) {
    throw new Error("0035 agent checkpoint schema mismatch: required foreign keys are missing or invalid");
  }
  ...
  if (index.length !== 1 || index[0].name !== "content_hash") {
    throw new Error("0035 agent checkpoint schema mismatch: content hash index is missing or invalid");
  }
  return true;
}
...
async function assertAgentCheckpointExternalSqliteSchema(storage) {
  ...
  for (const [table, shape] of Object.entries(AGENT_CHECKPOINT_TABLE_SHAPES)) {
    ...
    if (
      ...
      throw new Error(`0035 agent checkpoint schema mismatch for ${table}`);
    }
  }
  ...
  if (!agentCheckpointSqliteForeignKeysMatch(fks) || index.length !== 1 || index[0].name !== "content_hash") {
    throw new Error("0035 agent checkpoint schema mismatch: required foreign key or index is invalid");
  }
}
  ...
  for (const [table, shape] of Object.entries(AGENT_CHECKPOINT_TABLE_SHAPES)) {
    ...
    for (const column of columns) {
      ...
      if (
        ...
        throw new Error(`0035 agent checkpoint schema mismatch for ${table}.${column.name}`);
      }
    }
    ...
    if (JSON.stringify((pkResult.rows ?? []).map((row) => row.name)) !== JSON.stringify(shape.primaryKey)) {
      throw new Error(`0035 agent checkpoint schema mismatch for ${table} primary key`);
    }
  }
...
function buildMigrations(context) {
  ...
  return [
    ...
    {
      id: "0035_agent_checkpoints",
      name: "Add content-addressed generic agent checkpoints",
      checksum: "packages/db/migrations/0035_agent_checkpoints.sql",
      isApplied: (sqlite) => {
        if (
